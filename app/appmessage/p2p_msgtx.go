...
// Outpoint defines a consensus data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new consensus transaction outpoint point with the
...
// TxIn defines a consensus transaction input.
type TxIn struct {
...
// NewTxIn returns a new consensus transaction input with the provided
...
// TxOut defines a consensus transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new consensus transaction output with the provided
...
// MsgTx implements the Message interface and represents a consensus tx message.
