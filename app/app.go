...
import (
	...
	"github.com/consensus-network/consensusd/infrastructure/os/execenv"
	...
	"github.com/consensus-network/consensusd/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "consensusdsvc",
	DisplayName: "Consensusd Service",
	Description: "Downloads and stays synchronized with the Consensus blockDAG and " +
		"provides DAG services to applications.",
}

type consensusdApp struct {
	cfg *config.Config
}

// StartApp starts the consensusd app, and blocks until it finishes running
func StartApp() error {
...
func (app *consensusdApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start consensusd: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down consensusd...")
		...
		log.Infof("Consensusd shutdown complete")
	}()
