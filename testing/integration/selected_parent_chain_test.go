...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of consensusd instances
	consensusd1, consensusd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := consensusd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In consensusd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, consensusd2)
	}

	// Connect the two consensusds. This should trigger sync
	// between the two nodes
	connect(t, consensusd1, consensusd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := consensusd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
