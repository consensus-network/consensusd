...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("consensusdsanity-consensusd-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("consensusd", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "CONSENSUSD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "CONSENSUSD-STDERR")
