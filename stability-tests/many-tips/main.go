...
import (
	...
	"github.com/consensus-network/consensusd/domain/consensus/utils/mining"
	"github.com/consensus-network/consensusd/util"
	"github.com/kaspanet/go-secp256k1"

	"github.com/consensus-network/consensusd/stability-tests/common"
	"github.com/consensus-network/consensusd/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("consensusd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("consensusd datadir: %s", dataDir)

	consensusdCmd, err := common.StartCmd("CONSENSUSD",
		"consensusd",
	...
	spawn("startNode-consensusdCmd.Wait", func() {
		err := consensusdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("consensusdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes consensusd closes ungracefully
				log.Errorf("consensusdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(consensusdCmd, "consensusdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	consensusminerCmd, err := common.StartCmd("MINER",
		"consensusminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("consensus-miner-Cmd.Wait", func() {
		err := consensusminerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(consensusminerCmd, "consensusminerCmd")
	return nil
}
