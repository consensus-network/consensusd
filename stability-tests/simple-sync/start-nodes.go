...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("consensusd-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("consensusd-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("CONSENSUSD-SYNCER",
		"consensusd",
	...
	syncedCmd, err := common.StartCmd("CONSENSUSD-SYNCED",
		"consensusd",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
