...
import (
	...
	"github.com/consensus-network/consensusd/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		consensusdMessage, err := client.Post(&protowire.ConsensusdMessage{Payload: &protowire.ConsensusdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := consensusdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	consensusdMessage := &protowire.ConsensusdMessage{}
	err := protojson.Unmarshal([]byte(response), consensusdMessage)
	if err != nil {
	...
	return marshalOptions.Format(consensusdMessage)
}
