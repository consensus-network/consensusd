...
// 2. A wrapper that implements isConsensusdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `ConsensusdMessage_` prefix and `Request` suffix
...
// generateConsensusdMessage generates a wrapped ConsensusdMessage with the given `commandValue`
func generateConsensusdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.ConsensusdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	consensusdMessage := reflect.New(reflect.TypeOf(protowire.ConsensusdMessage{}))
	consensusdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return consensusdMessage.Interface().(*protowire.ConsensusdMessage), nil
}
