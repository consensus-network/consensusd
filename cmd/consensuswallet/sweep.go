...
func sweep(conf *sweepConfig) error {
	...
	publicKeybytes, err := libconsensuswallet.PublicKeyFromPrivateKey(privateKeyBytes)
	if err != nil {
		return err
	}
	...
	UTXOs, err := libconsensuswallet.ConsensuswalletdUTXOsTolibconsensuswalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatCss(splitTransactions[i].Outputs[0].Value), " CSS")
		totalExtracted = totalExtracted + splitTransactions[i].Outputs[0].Value
	}

	fmt.Println("\nTotal Funds swept (including transaction fees):")
	fmt.Println("\t", utils.FormatCss(totalExtracted), " CSS")
