...
// Takes in a string representation of the Css value to convert to Sompi
func TestCssToSompi(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := CssToSompi(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := CssToSompi(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateCSSAmountFormat(testCase)
