package libconsensuswallet_test
	...
	testutils.ForAllPaths(t, func(t *testing.T, version uint32) {
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
				...
				path := "m/1/2/3"
				address, err := libconsensuswallet.Address(params, publicKeys, minimumSignatures, path, ecdsa)
				if err != nil {
				...
				isFullySigned, err := libconsensuswallet.IsTransactionFullySigned(unsignedTransaction)
				if err != nil {
				...
				_, err = libconsensuswallet.ExtractTransaction(unsignedTransaction, ecdsa)
				...
				signedTxStep1, err := libconsensuswallet.Sign(params, mnemonics[:1], unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				isFullySigned, err = libconsensuswallet.IsTransactionFullySigned(signedTxStep1)
				if err != nil {
					t.Fatalf("IsTransactionFullySigned: %+v", err)
				}
				...
				signedTxStep2, err := libconsensuswallet.Sign(params, mnemonics[1:2], signedTxStep1, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				extractedSignedTxStep2, err := libconsensuswallet.ExtractTransaction(signedTxStep2, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}

				signedTxOneStep, err := libconsensuswallet.Sign(params, mnemonics[:2], unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				extractedSignedTxOneStep, err := libconsensuswallet.ExtractTransaction(signedTxOneStep, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}
		...
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
				...
				for i := 0; i < numKeys; i++ {
					...
					publicKeys[i], err = libconsensuswallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], false, version)
					if err != nil {
						t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
					}
				}
				...
				signedTx, err := libconsensuswallet.Sign(params, mnemonics, unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				tx, err := libconsensuswallet.ExtractTransaction(signedTx, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}
...
func TestMaxSompi(t *testing.T) {
	testutils.ForAllPaths(t, func(t *testing.T, version uint32) {
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			cfg.PreDeflationaryPhaseBaseSubsidy = 20e6 * constants.SompiPerConsensus
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libconsensuswallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], false, version)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			selectedUTXOsForTxWithLargeInputAmount := []*libconsensuswallet.UTXO{
				{
					Outpoint: &externalapi.DomainOutpoint{
			...
			unsignedTxWithLargeInputAmount, err := libconsensuswallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
				[]*libconsensuswallet.Payment{{
					Address: address,
					Amount:  10,
			...
			signedTxWithLargeInputAmount, err := libconsensuswallet.Sign(params, mnemonics, unsignedTxWithLargeInputAmount, false, version)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			txWithLargeInputAmount, err := libconsensuswallet.ExtractTransaction(signedTxWithLargeInputAmount, false)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
			...
			selectedUTXOsForTxWithLargeInputAndOutputAmount := []*libconsensuswallet.UTXO{
				{
					Outpoint: &externalapi.DomainOutpoint{
			...
			unsignedTxWithLargeInputAndOutputAmount, err := libconsensuswallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
				[]*libconsensuswallet.Payment{{
					Address: address,
					Amount:  22e6 * constants.SompiPerConsensus,
				}}, selectedUTXOsForTxWithLargeInputAndOutputAmount)
			if err != nil {
				t.Fatalf("CreateUnsignedTransactions: %+v", err)
			}

			signedTxWithLargeInputAndOutputAmount, err := libconsensuswallet.Sign(params, mnemonics, unsignedTxWithLargeInputAndOutputAmount, false, version)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			txWithLargeInputAndOutputAmount, err := libconsensuswallet.ExtractTransaction(signedTxWithLargeInputAndOutputAmount, false)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
