...
import (
	...
	"github.com/consensus-network/consensusd/cmd/consensuswallet/libconsensuswallet/bip32"
...
func create(conf *createConfig) error {
	...
	fmt.Printf("Notice the above is neither a secret key to your wallet " +
		"(use \"consensuswallet dump-unencrypted-data\" to see a secret seed phrase) " +
		"nor a wallet public address (use \"consensuswallet new-address\" to create and see one)\n\n")
	...
	if len(signerExtendedPublicKeys) > 0 {
		cosignerIndex, err = libconsensuswallet.MinimumCosignerIndex(signerExtendedPublicKeys, extendedPublicKeys)
		if err != nil {
			return err
		}
	}
